...
function GoogleServiceMap() {
  ...
  useEffect(() => {
    ...
    if (!window.google) {
      ...
      const apiKey = process.env.REACT_APP_GOOGLE_MAPS_API_KEY || 'AIzaSyCf2iRYkz881xx0UQaLxl0ltrklNVZhNpU';
  ...
  const fetchServiceCenters = async () => {
    try {
      ...
      const centersWithCoords = response.data.map((vendor) => ({
        id: vendor._id,
        name: vendor.name,
  ...
  }, [mapLoaded, serviceCenters, userLocation, initializeMap]);
